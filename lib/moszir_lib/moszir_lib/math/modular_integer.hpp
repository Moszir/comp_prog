...
public:
    ...
    Modular& operator--() { return *this -= 1; }
    Modular operator++(int) { Modular result(*this); operator++(); return result; } // NOLINT it's a problem if the return type is const, and also if it's not...
    Modular operator--(int) { Modular result(*this); operator--(); return result; } // NOLINT it's a problem if the return type is const, and also if it's not...
...
template <typename T> bool operator==(const Modular<T>& lhs, const Modular<T>& rhs) { return lhs.value_ == rhs.value_; }
